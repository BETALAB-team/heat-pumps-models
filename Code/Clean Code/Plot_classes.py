...
def barplot(model,KPI,font,k):
    ...
    #Create barplot 1 
    axs1.bar(br1, np.array(R2_stat), width = barWidth, color = "#13315c",edgecolor="black",label = "Steady state")
    axs1.bar(br2, np.array(R2_all), width = barWidth, color = "#fca151",edgecolor="black",hatch ="\\",label = 'All operative conditions')
    mean_stat = np.mean(R2_stat)
    mean_all = np.mean(R2_all)
    axs1.axhline(mean_stat, color="#012a4a", linestyle="--",linewidth = 4, label=f"$Mean_{{mod,ss}}$ = {mean_stat:.2f}")
    axs1.axhline(mean_all, color= "#ff7f0e", linestyle="--", linewidth = 4, label=f"$Mean_{{mod,all}}$ = {mean_all:.2f}")
    ...
    plt.tight_layout()
    # plt.savefig(os.path.join('..',"Result Analysis","New models results","New_Models_R2_av_mid_NO_GSHP.svg"))
    plt.close()
    
#%% Plot 2 rMAE
    figure2, axs2 = plt.subplots(1,figsize = (19,9.5))
    
    axs2.bar(br1, np.array(MAE_stat), width = barWidth, color = "#13315c",edgecolor="black",label = "Steady State")
    axs2.bar(br2, np.array(MAE_all), width = barWidth, color = "#fca151",edgecolor="black",hatch ="\\",label = "All operative conditions")
    mean_stat = np.mean(MAE_stat)
    mean_all = np.mean(MAE_all)
    axs2.axhline(mean_stat, color="#012a4a", linestyle="--",linewidth = 4, label=f"$Mean_{{mod,ss}}$ = {mean_stat:.2f}"+ " %")
    axs2.axhline(mean_all, color= "#ff7f0e", linestyle="--", linewidth = 4,label=f"$Mean_{{mod,all}}$ = {mean_all:.2f}"+ " %")
    ...
    plt.tight_layout()
    # plt.savefig(os.path.join('..',"Result Analysis","New models results","New_Models_rMAE_is_mid_NO_GHSP.svg"))
    plt.close()
...
#%%Plot 3 cRMSE
    ...
    axs2b.bar(br1, np.array(cRMSE_stat), width = barWidth,color = "#13315c", edgecolor="black", label = "Steady State")
    axs2b.bar(br2, np.array(cRMSE_all), width = barWidth, color = "#fca151",edgecolor="black",hatch ="\\" , label = 'All operative conditions')
    mean_stat = np.mean(cRMSE_stat)
    mean_all = np.mean(cRMSE_all)
    axs2b.axhline(mean_stat, color="#012a4a", linestyle="--",linewidth = 4, label=f"$Mean_{{mod,ss}}$ = {mean_stat:.2f}" + " %")
    axs2b.axhline(mean_all, color= "#ff7f0e", linestyle="--", linewidth = 4,label=f"$Mean_{{mod,all}}$ = {mean_all:.2f}"+ " %")
    ...
    axs2b.set_ylabel("CVRMSE [%]",fontsize = font)
    # axs2b.set_ylim(0,1)
    ...
    plt.tight_layout()
    # plt.savefig(os.path.join('..',"Result Analysis","New models results","New_Models_cRMSE_av_mid_NO_GSHP.svg"))
    plt.close()
...
#%% Scatter plot SCOP and Error
    ...
    if k == 1:
        ...
        markers = ['o']*8 +['s']*2 +['^']*3+['D']*1+['o']*6
        label = ["A05"]*8 + ["B10"]*2+["C10"]*3+["D12"]*1+["A12"]*6
        colors = ["red"]*8 +["blue"]*2+["purple"]*3+["grey"]*1+["orange"]*6
    ...
    axs4[0].text( 4.9, 5.7, "+10%",fontsize = font)
    axs4[0].plot([0, 10], [0, 9], "k--", label = "Error +20%")
    axs4[0].text( 5.1, 4.5, "-10%",fontsize = font)
    axs4[0].set_xlim(3,6)
    axs4[0].set_ylim(3,6)
    ...
    axs4[1].text( 4.9, 5.7, "+10%",fontsize = font)
    axs4[1].plot([0, 10], [0, 9], "k--", label = "Error +20%")
    axs4[1].text( 5.1, 4.5, "-10%",fontsize = font)
    axs4[1].set_xlim(3,6)
    axs4[1].set_ylim(3,6)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('..',"Result Analysis","New models results","SCOP_av_mid_NO_GSHP.svg"))
    plt.close()
...
# KPIs = pd.read_csv(os.path.join('..',"Result Analysis","New models results","KPIs_average_start_midnight.csv"))
# barplot(model,KPIs,20,1)      

KPIs = pd.read_csv(os.path.join('..',"Result Analysis","New models results","KPIs_average_start_midnight -NO_GSHP.csv"))
barplot(model,KPIs,20,1)      

# KPIs = pd.read_csv(os.path.join('..',"Result Analysis","New models results","KPIs_average_start_midnight_sen.csv"))
