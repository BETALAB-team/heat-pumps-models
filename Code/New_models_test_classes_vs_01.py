...
for dev in devices:
    ...
    # relative = za/len(HP.test_fil["Status"])*100
    # relative = len(HP.test)/69696*100
    relative = len(HP.test_fil["PLR"] == 0)
    count.append(relative)
    
    #Evaluation of mean electrical consuption
    # onoff = HP.test_fil.loc[(HP.test_fil["PLR"]>=0.23) & (HP.test_fil["PLR"]<=0.27)]
    # meanP = onoff["Pow [kW]"].mean()
