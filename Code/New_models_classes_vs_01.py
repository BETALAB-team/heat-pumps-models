...
class Heat_Pumps():
    ...
    def __init__(self,device):
        ...
        self.name, catalogue_data = device
    ...
    def new_model_fit(self):        
        ...
        for i in range(4):
            if i == 0:
               self.test_fil = self.test[self.test['Status'] == 'STATIONARY']
                ...
                self.test_fil = self.test[(self.test['Status'] == 'ACCELERATION') | (self.test['Status'] == 'DECELERATION')
                          | (self.test['Status'] == 'STATIONARY')]
            elif i == 2:
                self.test_fil = self.test[(self.test['Status'] == 'ACCELERATION') | (self.test['Status'] == 'DECELERATION')
                          | (self.test['Status'] == 'STATIONARY') | (self.test['Status'] == 'DEF')]
            elif i == 3:
                self.test_fil = self.test[(self.test['Status'] == 'ACCELERATION') | (self.test['Status'] == 'DECELERATION')
            ...
            PLR =  self.train["PLR"]
             ...
             # #Normalize the thermodinamics variables - self.testerimental Data
            ...
            #Control Approach
            # X_train,X_test,Y_train,Y_test = train_test_split(X_test, self.test_fil["Pow [kW]"]/self.train["Pow full [kW]"], test_size = 0.20)
            ...
            # # Pow_pred = model_reg_P.predict(X_test)* self.test["Pow full [kW]"]
